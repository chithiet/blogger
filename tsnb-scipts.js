...
function getFirstImage(t, i) {
    var r = $("<div>").html(t).find("img:first").attr("src");
    $("<div>").html(t).find("img").each(function () {
        var t = $(this).attr("src");
        if (t) return r = t, !1
    }), k = null != r ? r : $("<div>").html(t).find("img:first").attr("src");
    var n = k.lastIndexOf("/") || 0,
        s = k.lastIndexOf("/", n - 1) || 0,
        a = k.substring(0, s),
        f = k.substring(s, n),
        g = k.substring(n);
    return (f.match(/\/s[0-9]+/g) || f.match(/\/w[0-9]+/g) || "/d" == f) && (f = "/w72-h72-p-k-no-nu"), a + f + g
}
